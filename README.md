...
| [`06_experiments/loss_curve_compare.py`](06_experiments/loss_curve_compare.py) | 生成 ANN 与 XGBoost 的 Loss 曲线对比图，补充迭代模型收敛分析 | Loss 曲线实验 |
...
| 模型            | 高斯噪声 0.00 | 高斯噪声 0.15 | 均匀噪声 0.00 | 均匀噪声 0.15 |
| ------------- | --------: | --------: | --------: | --------: |
| KNN           | 0.9211 | 0.9160 | 0.9211 | 0.9193 |
| SVM           | 0.9335 | 0.9303 | 0.9335 | 0.9320 |
| Random Forest | 0.9233 | 0.9257 | 0.9233 | 0.9257 |
| ANN           | 0.9262 | 0.9232 | 0.9262 | 0.9268 |
| XGBoost       | 0.9262 | 0.9286 | 0.9262 | 0.9279 |

鲁棒性实验通过向训练数据加入不同类型、不同强度的噪声，并在加噪训练集上重新训练模型，再使用干净测试集进行评价。实验噪声包括高斯噪声和均匀噪声，噪声强度从 0.00 增加到 0.15，并通过多随机种子重复实验统计平均准确率。

从结果看，SVM 在两类噪声条件下均保持最高准确率，说明其分类边界在训练数据扰动后仍较稳定。KNN 在高斯噪声强度为 0.15 时准确率下降到 0.9160，表明距离度量模型对训练特征扰动相对敏感。Random Forest 和 XGBoost 在较高噪声条件下没有明显退化，说明树集成模型对局部训练扰动具有一定适应能力。ANN 在均匀噪声条件下保持较稳定表现，整体准确率波动较小。
...
#### 1. 准确率对比
![accuracy](08_results/paper_figures/accuracy_comparison_paper.png)

#### 2. 速度对比
![speed](08_results/paper_figures/speed_comparison_paper.png)

#### 3. 鲁棒性分析
![robustness-gaussian](08_results/paper_figures/robustness_training_noise_gaussian.png)

![robustness-uniform](08_results/paper_figures/robustness_training_noise_uniform.png)

#### 4. 过拟合分析
![overfit](08_results/paper_figures/overfit_gap_comparison_paper.png)

#### 5. Loss 曲线对比
![loss-comparison](08_results/paper_figures/loss_curve_comparison_paper.png)

#### 6. 混淆矩阵分析
![confusion-course](08_results/paper_figures/confusion_matrix_course_models.png)

![confusion-extra](08_results/paper_figures/confusion_matrix_extra_models.png)
